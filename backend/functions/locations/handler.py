...
def handler(event, context):
    """Reusable venue name/address shortcuts (name, address, order -
    drag-to-reorder, same pattern as Coaches). Schedule's Away-game form
    picks a location_id from this list instead of the admin retyping the
    same venue's location/address text on every game.
    """
    method = event["requestContext"]["http"]["method"]
    location_id = (event.get("pathParameters") or {}).get("location_id")
    ...
    if method in ("POST", "PUT"):
        ...
        body["location_id"] = location_id or body.get("location_id") or str(uuid.uuid4())
        body.setdefault("order", int(time.time() * 1000))
        table.put_item(Item=body)
        return _response(200, body)

    if method == "DELETE" and location_id:
        table.delete_item(Key={"location_id": location_id})
